...
if ( have_posts() ) {
  ...
  ?>
        ...
        <?php if ( ! empty( $post->post_parent ) ) : ?>
          <p class="page-title"><a href="<?php echo esc_url( get_permalink( $post->post_parent ) ); ?>" title="<?php echo esc_attr( sprintf( __( 'Return to %s', 'manoa2018' ), strip_tags( get_the_title( $post->post_parent ) ) ) ); ?>" rel="gallery">
          <?php
          /* translators: %s - title of parent post */
          printf( __( '<span class="meta-nav">&larr;</span> %s', 'manoa2018' ), get_the_title( $post->post_parent ) );
          ?>
          </a></p>
        <?php endif; ?>
          ...
          <div class="entry-meta">
            <?php
              printf(
                __( '<span class="%1$s">By</span> %2$s', 'manoa2018' ),
              ...
              <?php
              printf(
                __( '<span class="%1$s">Published</span> %2$s', 'manoa2018' ),
              ...
              if ( wp_attachment_is_image() ) {
                ...
                printf(
                  __( 'Full size is %s pixels', 'manoa2018' ),
                  sprintf(
                    '<a href="%1$s" title="%2$s">%3$s &times; %4$s</a>',
                    esc_url( wp_get_attachment_url() ),
                    esc_attr( __( 'Link to full-size image', 'manoa2018' ) ),
                    $metadata['width'],
                    $metadata['height']
                  )
                );
  ...
  ?>
            <p class="attachment"><a href="<?php echo esc_url( $next_attachment_url ); ?>" title="<?php the_title_attribute(); ?>" rel="attachment">
              <?php
              /**
               * Filter the Manoa 2018 default attachment width.
               *
               */
              $attachment_width = apply_filters( 'manoa2018_attachment_size', 900 );
              /**
               * Filter the Manoa 2018 default attachment height.
               *
               */
              $attachment_height = apply_filters( 'manoa2018_attachment_height', 900 );
